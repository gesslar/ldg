...
static void logon2(string str) {
     ...
     * our first title !
     */
    if(level != -1) input_to("check_password", 1, "Password: ");
    else input_to("new_password", 1, "Password: ");
    if(name == "guest") write("(just CR) ");
...
static void check_password(string input)
{
    write("\n");
    ...
    if(name != "guest")
    {
        int result = AUTH_D->authenticate(name, input) ;
 ...
 */
static void new_password(string input, string password)
{
    write("\n");
    if(!input || input == "")
    {
    ...
    if(sizeof(input) < 6)
    {
        ...
        input_to("new_password", 1, "Password: ");
        return;
    }
    ...
    {
        password = input;
        input_to("new_password", 1, password, "Password: (again) ") ;
        return;
    }
    ...
    AUTH_D->update_password(name, password) ;
 ...
 */
static int change_password(string password)
{
    ...
    if(!AUTH_D->authenticate(name, password))
    {
        write("Wrong old password.\n");
        return 1;
    }

    input_to("change_password2", 1, "New password: ");
    return 1;
}

static void change_password2(string input, string password)
{
    if(!input || input == "")
    {
    ...
    if(password = 0)
    {
        input_to("change_password2", 1, "Again: ", input);
        return;
    }
...
int query_player_object() { return 1 ; }
